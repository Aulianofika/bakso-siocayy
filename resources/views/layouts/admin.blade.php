...
<head>
  ...
  <style>
    ...
    :root {
      ...
      --shadow-glow: 0 0 15px rgba(34, 197, 94, 0.2);
    ...
    body {
      ...
      background-color: #f8fcf9; /* Lighter solid background */
      /* Removed heavy radial gradients for performance */
      color: var(--text-main);
      font-size: clamp(0.9rem, 1vw + 0.5rem, 1.05rem); /* Responsive base font size */
    }
    ...
    @media (max-width: 991.98px) {
      .nav-item:hover { transform: none; } /* Disable hover movement on mobile */
    }
    .nav-item:hover i {
      transform: scale(1.1);
    }
...
<body>
  ...
  <div class="app-container">
    ...
    <aside id="sidebar" class="sidebar">
      ...
      <nav class="nav-menu">
        ...
        @endphp
        <a class="nav-item {{ $isDashboardActive ? 'active' : '' }} collapsed" 
        ...
        <div class="collapse {{ $isDashboardActive ? 'show' : '' }} ps-3" id="dashboardSubmenu">
            <div class="nav-menu mt-1 gap-1 border-start border-2 ms-3 ps-2" style="border-color: var(--border-subtle) !important;">
                <a href="{{ route('admin.dashboard', ['type' => 'bakso']) }}" 
                   ...
                   <i class="bi bi-circle-fill text-success" style="font-size: 0.4rem; opacity: {{ $isDashboardActive && $dashType == 'bakso' ? '1' : '0.5' }}"></i>
                   <span> Bakso</span>
                </a>
                ...
                <a href="{{ route('admin.dashboard', ['type' => 'kopi']) }}" 
                   ...
                   style="font-size: 0.95rem;">
                   <i class="bi bi-circle-fill text-success" style="font-size: 0.4rem; opacity: {{ $isDashboardActive && $dashType == 'kopi' ? '1' : '0.5' }}"></i>
                   <span> Kopi</span>
                </a>
                ...
                <a href="{{ route('admin.dashboard', ['type' => 'all']) }}" 
                   ...
                   style="font-size: 0.95rem;">
                   <i class="bi bi-circle-fill text-success" style="font-size: 0.4rem; opacity: {{ $isDashboardActive && ($dashType == 'all' || !$dashType) ? '1' : '0.5' }}"></i>
                   <span>Semua </span>
                </a>
            </div>
        </div>

        <div class="text-muted small fw-bold mt-3 mb-2 px-3 text-uppercase" style="font-size: 0.8rem; letter-spacing: 1px;">Master Data</div>
        ...
        <a href="{{ route('admin.customers.index') }}" class="nav-item {{ request()->is('admin/customers*') ? 'active' : '' }}">
          <i class="bi bi-people"></i>
          <span>Pelanggan</span>
        </a>

        <div class="text-muted small fw-bold mt-3 mb-2 px-3 text-uppercase" style="font-size: 0.8rem; letter-spacing: 1px;">Transaksi</div>
        ...
        @endphp
        <a class="nav-item {{ $isOrdersActive ? 'active' : '' }} collapsed" 
           href="#ordersSubmenu" 
           data-bs-toggle="collapse" 
           role="button" 
        ...
        <div class="collapse {{ $isOrdersActive ? 'show' : '' }} ps-3" id="ordersSubmenu">
            <div class="nav-menu mt-1 gap-1 border-start border-2 ms-3 ps-2" style="border-color: var(--border-subtle) !important;">
                <a href="{{ route('admin.orders.index', ['type' => 'bakso']) }}" 
                   ...
                   style="font-size: 0.95rem;">
                   <i class="bi bi-circle-fill text-success" style="font-size: 0.4rem; opacity: {{ $isOrdersActive && $orderType == 'bakso' ? '1' : '0.5' }}"></i>
                ...
                <a href="{{ route('admin.orders.index', ['type' => 'kopi']) }}" 
                   ...
                   style="font-size: 0.95rem;">
                   <i class="bi bi-circle-fill text-success" style="font-size: 0.4rem; opacity: {{ $isOrdersActive && $orderType == 'kopi' ? '1' : '0.5' }}"></i>
                ...
                <a href="{{ route('admin.orders.index', ['type' => 'all']) }}" 
                   ...
                   style="font-size: 0.95rem;">
                   <i class="bi bi-circle-fill text-success" style="font-size: 0.4rem; opacity: {{ $isOrdersActive && ($orderType == 'all' || !$orderType) ? '1' : '0.5' }}"></i>
                   <span>Semua Pesanan</span>
                </a>
            </div>
        </div>

        <a href="{{ route('admin.shipments.index') }}" class="nav-item {{ request()->is('admin/shipments*') ? 'active' : '' }}">
          <i class="bi bi-truck"></i>
          <span>Pengiriman</span>
        </a>
        
        <a href="{{ route('admin.stock-incidents.index') }}" class="nav-item {{ request()->is('admin/stock-incidents*') ? 'active' : '' }}">
    ...
    <div class="main-content">
      ...
      <header class="topbar">
        ...
        <div class="topbar-right">
          <!-- Notification Bell -->
          <a href="{{ route('admin.orders.index', ['status_order' => 'Pending']) }}" class="notify-btn" title="Pesanan Baru">
            <i class="bi bi-bell-fill"></i>
            @if(isset($newOrdersCount) && $newOrdersCount > 0)
              <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger border border-white" style="font-size: 0.75rem;">
            ...
            <ul class="dropdown-menu dropdown-menu-end border-0 shadow-lg p-2 rounded-3 mt-2">
              <li>
                <form action="{{ route('logout') }}" method="POST">
                  @csrf
                  <button class="dropdown-item rounded-2 text-danger fw-bold d-flex align-items-center gap-2 py-2" type="submit">
  ...
  <script>
    ...
    // Close sidebar on route change (for cleaner mobile feeling if using SPA features, though this is SSR)
