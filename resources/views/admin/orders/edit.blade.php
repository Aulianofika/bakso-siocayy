        ...
        <form action="{{ route('admin.orders.update', $order->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="status_payment" class="form-select" required>
                    <option value="Belum Bayar" {{ $order->status_payment == 'Belum Bayar' ? 'selected' : '' }}>Belum Bayar
                    </option>
                    <option value="Menunggu Verifikasi" {{ $order->status_payment == 'Menunggu Verifikasi' ? 'selected' : '' }}>Menunggu Verifikasi
                    </option>
